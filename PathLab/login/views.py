...
def log_in(request): #reuest for login page
	if(len(request.session.keys())!=0):
		...
		elif "agent" in request.session:
			return redirect('/agent/')
		elif "nomi" in request.session:
			return redirect('/nom/')
...
def login_auth(request): #authorization of login
	...
	try:
		...
		if role == 'user':
			cur.execute('select count(*) from user where ID=%s and password=%s and emailid=%s',(login,pswd,emailid))
		elif role == 'nomi':
			cur.execute('select count(*) from nominee where ID=%s and password=%s and emailid=%s',(login,pswd,emailid))
		elif role == 'agent':
			cur.execute('select count(*) from agent where ID=%s and password=%s and emailid=%s and working="YES" or working="L"',(login,pswd,emailid))
		else:
...
def reg(request): # sign up form for users
	...
	gender.lower()
	try:
		cur.execute('insert into user values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(login,fname,lname,dob,age,gender,hno,
